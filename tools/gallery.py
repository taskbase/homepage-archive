...
.smoke .cloud{border-radius:50%;filter:blur(64px)}
...
function paint(){
  ...
  style.setProperty('--era', eras[i]);
  style.setProperty('--era-next', eras[Math.min(eras.length - 1, i + 1)]);
  style.setProperty('--mix', (pos - i).toFixed(3));
}
