@Component({
  selector:'tbDeprecatedLanding',
  templateUrl: '/components/directives/tb-landing/deprecated/tb-landing-component.html',
  bindings: {
    somebinding: "=" // One Way Binding
  }
})
class TbDeprecatedLandingComponent {
  ...
  $onInit() {
    ...
    let numUsersPromise = this.UserService.numStudents();

    this.$q.all([numTeachersPromise, numUsersPromise]).then(resp => {
      this.counters = [{
        n: resp[1].data.count,
        title: "Lernende"
      }, {
        ...
        n: this.TaskService.numTasksTotal(),
        title: "Lerninhalte"
      }];
    });
  }
